 ...
 * they did not ask for. The four refusals are the licence guard, an existing row, no locale and no
 * tenant.
 */
final class CatalogueContributor
{
    ...
    public function contribute(Product $product, ?Barcode $barcode): ?GlobalProduct
    {
        ...
        $row = GlobalProduct::create([
            ...
            // **Photos are never contributed** (`barcode-and-catalog.md`). The terms grant a licence
            // over the text fields only, so `image_path` stays null however good the photograph is.

...
- **A controller validates; a sheet uses `Validator` directly.** `ValidatesRequests` is declared `on MagicController`, so a `State` cannot mix it. A bottom sheet that returns a draft to a parent validates with `Validator.make(data, rules)` and keeps the errors in its own state; three of them do.

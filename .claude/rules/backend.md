...
**The client currently enforces none of them.** Nothing under `lib/` limits a name's length, so the app accepts 300 characters and the user meets a 422 where a field error belongs. A bound added or changed here gets its counterpart in the same PR; `.claude/rules/flutter-app.md` carries the client half.

...
- **`text-destructive`, for the same reason and one step less obvious.** `destructive` IS declared here, and `design:sync` emits `bg-destructive`, `text-on-destructive` and `bg-destructive-container` from it and no text alias at all. So an error sentence written in it renders at full foreground weight, which reads as ordinary copy rather than as a refusal. `expired` equals `destructive` in this palette, by this file's own rule, so a red sentence is `text-expired`.

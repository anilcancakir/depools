...
- **`uppercase` on Turkish text, until Wind ships locale-aware casing.** The token calls Dart's `String.toUpperCase()` (`fluttersdk_wind-0.0.4/lib/src/parsers/text_transform_parser.dart:19`), which is locale-INSENSITIVE: it maps `i` to `I` and never to `İ`. Measured, not assumed: `süresi geçmiş` becomes `SÜRESI GEÇMIŞ`, `kiler` becomes `KILER`, `geniş` becomes `GENIŞ`. On a Turkey-first product that is wrong on every section header, and it is visible right now on the dates screen, the stock-take screen and the preview harness's own viewport label. The same parser's `capitalize` carries a second form of it: `substring(1).toLowerCase()` turns `İ` into `i` plus a combining dot. The fix belongs in Wind and is a PR there (see `docs/depools-system/open-decisions.md` D115); until it lands, prefer a sentence-case label over adding a new `uppercase` one, because every new one is another wrong heading.

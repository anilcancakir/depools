...
| A `magic_starter` control used with no `className`, reviewed in dark only | Pass `className: 'border border-color-border'` on anything whose SHAPE is carried by a fill, and check it in light. `MSSwitch`'s off track is `#E7E6EC` with a white thumb: on a white card the control's own edge is 1.21:1 and the whole switch nearly disappears, while dark mode looks perfect. Measured with a pixel scan, not by eye, after it had shipped on three screens. The hairline takes it to 1.67:1, which is an improvement and not a pass; DESIGN.md records what closing it needs. |

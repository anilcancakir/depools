...
| A screen with no `PreviewChrome.appMobile` preview | Add one. Fourteen screens were verified at catalog width only and two carried defects nobody could see: a `LotRow` meta row overflowing by 16 to 36px and a `CountRow` needing 370px of a 326px card. Neither is visible wide, because neither layout enters its narrow arrangement there. One phone preview per screen, and probe it on the first paint after a restart. |

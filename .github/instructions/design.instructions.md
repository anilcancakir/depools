...
| A stepper built as three bordered boxes in a row | Exactly ONE bordered box: the field keeps its border and the buttons flank it with none (Material's outlined-field-with-icons shape). Three bordered boxes give three heights and two radii and read as three unrelated controls. Do NOT instead move the border to the group and put a raw `WInput` inside: `WInput` needs an `Overlay` ancestor for its selection machinery and throws a build-phase `setState` from the preview harness. Both were tried. |
